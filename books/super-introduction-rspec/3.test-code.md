...
```
:::details 補足：1行目の**RSpec.describe**と**type: :model**について
- RSpec.describe
テストコードの先頭にある`RSpec.describe`とは、RSpecモジュールのdescribeメソッドを呼び出す処理を実行しています。
実はネスト内にある`describe`は、`RSpec.describe`のエイリアスなので、RSpecを先頭に付けなくてもテストコードとして成立します。
- type: :model
こちらは、RSpecに「このテストコードはモデルスペックです。」ということを伝えるために指定しています。
書く理由としては、typeごとに使用する**ヘルパー**や**設定**が定義されているため、typeを指定しないと予期せぬ挙動が発生してしまう可能性が出てきます。<br>
しかしこちらのコードについても、該当するスペックディレクトリの配下に置いておけば、自動的にtypeを推測してくれます。(今回の場合モデルスペックなので、`spec/models/`に置いておけば`type: :model`と判定)
```ruby:spec/rails_helper.rb
# 以下のコードでディレクトリ名からtypeを推測する機能を使用している
config.infer_spec_type_from_file_location!
```
なので正直な事を言うと、以下の省略したコードでもテストは問題なく通ります。
```ruby:例文(book_spec.rb)
describe "Book" do
# RSpecを省略＆modelsディレクトリの配下であれば、Bookクラスのモデルスペックテストとしてグループ化出来る
  describe "Book#title_with_author" do
  # Bookクラス内のtitle_with_authorメソッドのテストをグループ化
```
しかしこれでは、以下２つのコードということを認識しづらいです。
- RSpecのコードだということ
- モデルスペックを指定していること

なので**可読性**を高める為に、余程の事がない限り記述するべきだと思います。
:::

### context
`context`はテストをする前の**前提条件**を記述します。前提条件ごとにグループ化することで、状況別でテストケースを把握しやすくなります。
...
##### 正常系を書く必要性
正常系はアプリを期待通りに動作させるために必要不可欠なテストケースです。
アプリの動作保証に繋げることが出来ますし、プロジェクトの仕様を理解しやすくなります。(テストコード自体を仕様書化して全体の処理を把握することも可能)
#### 異常系
...
##### 異常系を書く必要性
異常系を書いておくことで、アプリの**安定性・信頼性の向上**を保証することが出来ます。
アプリを安全に運用するために必要不可欠なテストケースです。
### it
...
class Book < ApplicationRecord
  def title_with_author
    # 以下のコードを出力してテストが成功した場合、
    # テストコードの1行目に記述したBook.newも無事に作成出来ていることをチェック出来る
    "#{title} - #{author}"
  end
end
```
